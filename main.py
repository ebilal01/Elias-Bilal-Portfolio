...
async def submit_form(data: LungCancerData = Body(...)):
    try:
        ...
        print("number of matched patients for Air Quality Use  filter = " + row)
        print("number of matched patients for Alcohol Use  filter = " + row3 )
        print("number of matched patients for Sex filter = " + row2)
        # takes an average of the probability levels of each matched patient 
        Total = level_df.sum()
        Total3 = level3_df.sum()
        Total2 = level2_df.sum()

        print("total of all levels in Airquality filter =" + Total)
        print("total of all levels in Alcohol use filter =" + Total3)
        print("total of all levels in Sex filter =" + Total2)
        ...
        if row == 0 and row3 == 0:
            ave = (Total2/row2)
            print ("filter 2 average = " + ave)
            print ("filter2 levels = " + filter2_df["Level"])
